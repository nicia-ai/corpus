...
// src/auth.ts's plugin surface so the generated Drizzle schema covers
...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    // Mirror src/auth.ts: schema-affecting options only. Runtime
    // behavior (hooks, email) lives in src/auth.ts, not here.
