...
async function build(store: ReturnType<typeof ws>) {
  ...
  // Root doc — exists, but NOT under folder `a` (out of the corpus).
  await store.importDocumentAtPath({
    path: "top.md",
    markdown: "# Top",
    changedBy: by,
...
function linksOf(
  outline: Awaited<ReturnType<ReturnType<typeof ws>["corpusOutline"]>>,
  slug: string,
) {
  if (!outline.found) throw new Error("corpus not found");
...
describe("corpusOutline — derived paths + resolved link graph", () => {
  it("projects tree-ordered docs with derived paths", async () => {
    const w = ws();
    await build(w);
    const o = await w.corpusOutline(colSlug("k"));
  ...
  it("resolves every relative link flavour; external/anchor are not links", async () => {
    ...
    // Resolves to a real document that is OUTSIDE this corpus.
    ...
    // …including targets outside this corpus…
  ...
  it("an unknown corpus has no outline", async () => {
    const w = ws();
    expect(await w.corpusOutline(colSlug("nope"))).toEqual({
      found: false,
    });
  });
});
